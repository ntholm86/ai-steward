...
3. **skill suite** — migrates from `.acm/` to `.pea/` — the legacy system
...
The primary onboarding path: **operator drops raw context into `.acm/` → ai-steward
...
- **Cold start** — no prior retrospect has run (`.acm/retrospect.md` absent or empty)
